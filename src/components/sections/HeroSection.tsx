...
export function HeroSection() {
  const { isLoaded, selectHospital, selectedHospitalId, showMapOverlay, isMobileDevice, isMapFocused, unfocusMap } = useNaverMap('hero-map')
  ...
  const handleSelectHospital = (hospitalId: number) => {
    ...
    setIsMobileListOpen(false) // 모바일에서 선택 후 닫기
  }
  ...
  return (
    <section className="relative h-screen">
      ...
      <div className="absolute inset-0">
        <div id="hero-map" className="w-full h-full" />
        {/* Gradient Overlay for readability - pointer-events-none to allow map interaction */}
        ...
        {!isMobileDevice && isMapFocused && (
          <button
            onClick={unfocusMap}
            className="hidden lg:flex absolute top-24 right-8 z-30 bg-white/95 backdrop-blur-md rounded-xl shadow-lg px-4 py-3 items-center gap-2 hover:bg-white transition-colors"
          >
      ...
      <div
        ref={listRef}
        className="hidden lg:block absolute top-24 left-8 z-20 w-80 max-h-[calc(100vh-200px)] opacity-0"
      >
      ...
      <div className="lg:hidden absolute bottom-0 left-0 right-0 z-20">
        {/* Collapsed State - Show selected hospital */}
        <div
      ...
      {/* Scroll Indicator - Hidden on mobile when list is open */}
      <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-10 pointer-events-none transition-opacity duration-300 ${isMobileListOpen ? 'opacity-0' : 'opacity-100'} hidden lg:flex`}>
